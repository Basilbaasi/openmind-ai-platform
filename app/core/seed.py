...
async def seed_sessions(session: AsyncSession) -> None:
    ...
    messages = [
        MessageRecord(
            ...
            session_id="sess_1",
            role="system",
            content="You are an expert SQL engineer. Return only optimal queries.",
        ),
        MessageRecord(
            id="m2",
            session_id="sess_1",
            role="user",
            content="Optimize this query: SELECT * FROM orders WHERE user_id IN (SELECT id FROM users WHERE signup_date > '2026-01-01')",
        ),
        MessageRecord(
            id="m3",
            session_id="sess_1",
            role="assistant",
            content="You can optimize this query by converting the subquery into an `INNER JOIN`, which allows the database planner to use indexes more efficiently:\n\n```sql\nSELECT o.*\nFROM orders o\nINNER JOIN users u ON o.user_id = u.id\nWHERE u.signup_date > '2026-01-01';\n```\n\nEnsure there is a composite index on `users(signup_date, id)` and an index on `orders(user_id)`.",
        ),
        MessageRecord(
            id="m4",
            session_id="sess_2",
            role="system",
            content='Analyze user queries and categorize into: Refund, Technical, Billing, shipping. Return JSON as {"category": string, "confidence": float, "urgency_level": "low" | "medium" | "high"}',
        ),
        MessageRecord(
            id="m5",
            session_id="sess_2",
            role="user",
            content="I've been charged twice for my subscription this month and need this sorted immediately!",
        ),
        MessageRecord(
            id="m6",
            session_id="sess_2",
...
async def seed_memory(session: AsyncSession) -> None:
    ...
    nodes = [
        MemoryNodeRecord(
            id="mem_1",
            label="Semantic Search Project",
            tier="Semantic",
            category="Project",
            value="Focuses on deploying dense retrievers combined with re-ranking filters.",
        ),
        MemoryNodeRecord(
            id="mem_2",
            label="Llama 3 Local Host",
            tier="Conversation",
            category="Infrastructure",
            value="Running Llama 3 8B locally on CUDA 12.2, drawing ~120W, quantized to Q4_K_M.",
        ),
        MemoryNodeRecord(
            id="mem_3",
            ...
            tier="Long-Term",
            category="Parameter",
            value="Determined 512-character overlapping chunks work best for technical documentation retrieval.",
        ),
        MemoryNodeRecord(
            id="mem_4",
            label="BGE Large EN Model",
            tier="Semantic",
            category="Model",
            value="BGE Large EN v1.5 offers the best cross-encoder similarity accuracy.",
        ),
        MemoryNodeRecord(
            id="mem_5",
            ...
            tier="Conversation",
            category="Security",
            value="API keys rotate every 30 days. Fallback keys are triggered if key validation throws 401.",
        ),
    ]
    for node in nodes:
...
async def seed_memory_logs(session: AsyncSession) -> None:
    ...
    logs = [
        MemoryLogRecord(
            ...
            tier="Conversation",
            operation="Write",
            text="Successfully captured user's SQL preference and cached in session memory.",
        ),
        MemoryLogRecord(
            id="mlog_2",
            tier="Semantic",
            operation="Read",
            text="Query 'dense retrieval' hit node 'Semantic Search Project' with cosine similarity score: 0.89.",
        ),
        MemoryLogRecord(
            id="mlog_3",
            tier="Long-Term",
            operation="Consolidate",
            text="Consolidated 15 similar conversation sessions into core Long-Term node 'Llama 3 Local Host' after clustering analysis.",
        ),
        MemoryLogRecord(
            id="mlog_4",
            tier="Semantic",
            operation="Prune",
            text="Pruned 3 stale short-term nodes (decay threshold > 0.85).",
        ),
    ]
    for log in logs:
...
async def seed_workflows(session: AsyncSession) -> None:
    ...
    steps = [
        WorkflowStepRecord(
            ...
            type="Condition",
            config=json.dumps({"regex": "(?i)exception|error"}),
            order=0,
        ),
        WorkflowStepRecord(
            id="s1_2",
            ...
            type="Memory_Fetch",
            config=json.dumps({"lookup_tier": "Semantic", "threshold": 0.75}),
            order=1,
        ),
        WorkflowStepRecord(
            id="s1_3",
            workflow_id="wf_1",
            name="Generate Solution Proposal",
            type="LLM",
            config=json.dumps({"model": "llama3-8b-instruct", "temperature": 0.1}),
            order=2,
        ),
        WorkflowStepRecord(
            id="s1_4",
            workflow_id="wf_1",
            name="Request Engineer Approval",
            type="Human_Approval",
            config=json.dumps({"notification": "slack"}),
            order=3,
        ),
        WorkflowStepRecord(
            id="s2_1",
            workflow_id="wf_2",
            name="Fetch Fresh Records",
            type="API_Call",
            config=json.dumps({"endpoint": "https://db.internal/v1/sync"}),
            order=0,
        ),
        WorkflowStepRecord(
            id="s2_2",
            ...
            type="LLM",
            config=json.dumps({"model": "gpt-4o", "batch_size": 50}),
            order=1,
        ),
        WorkflowStepRecord(
            id="s2_3",
            workflow_id="wf_2",
            name="Writeback To Postgres",
            type="API_Call",
            config=json.dumps({"method": "PUT"}),
            order=2,
        ),
        WorkflowStepRecord(
            id="s3_1",
            workflow_id="wf_3",
            name="Gather Latency Metrics",
            type="Condition",
            config=json.dumps({"metric": "latency", "max_value": 2500}),
            order=0,
        ),
        WorkflowStepRecord(
            id="s3_2",
            ...
            name="Send Alerts",
            type="API_Call",
            config=json.dumps({"webhook_url": "https://pagerduty.internal/webhook"}),
            order=1,
        ),
    ]
    for step in steps:
...
async def seed_benchmarks(session: AsyncSession) -> None:
    ...
    benchmarks = [
        ...
        BenchmarkRecord(
            model_id="nvidia-ising-1.5-31b",
            model_name="NVIDIA Ising 1.5 31B",
            ttft_ms=80,
            tps=55.0,
            latency_ms=85,
            accuracy=90.2,
...
async def seed_logs(session: AsyncSession) -> None:
    ...
    logs = [
        LogEntryRecord(
            ...
            severity="INFO",
            source="CORE_VM",
            message="Starting OpenMind core VM execution loop... Core modules initialized cleanly.",
        ),
        LogEntryRecord(
            id="l2",
            severity="INFO",
            source="MODEL_INFRA",
            message="Successfully allocated 6.5GB VRAM for local model: Llama 3 8B Instruct.",
        ),
        LogEntryRecord(
            id="l3",
            severity="INFO",
            source="MEMORY_DB",
            message="Semantic index loaded successfully. 5 nodes indexed from vector DB store.",
        ),
        LogEntryRecord(
            id="l4",
            severity="WARN",
            source="API_GATEWAY",
            message="API key validation mismatch warning on endpoint `/v1/embeddings`.",
            metadata_json=json.dumps(
                {
                    "request_id": "req_3",
                    "origin_ip": "192.168.1.104",
                    "supplied_key_prefix": "om_invalid",
                }
            ),
        ),
        LogEntryRecord(
            ...
            severity="ERROR",
            source="ORCHESTRATOR",
            message="Workflow 'Data Enrichment Pipeline' returned execution failure on step 3. Connection refused from db.internal.",
        ),
        LogEntryRecord(
            id="l6",
            severity="INFO",
            source="INGEST_PIPE",
            message="Successfully chunked Markdown source 'System_Architecture_Layout.md' into 12 dense nodes.",
        ),
    ]
    for log in logs:
        await session.merge(log)
